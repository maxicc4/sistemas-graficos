        ...
        void main(void) {
            ...
            position=vec3(uv.x-0.5, -0.25, uv.y-0.5)*2.0;
            // distancia al origen
            float distancia = sqrt(position.x*position.x+position.z*position.z);
            float amplitud = (1.0/distancia)*0.01;
            if (amplitud > 0.2) {
                amplitud = 0.2;
            }
            position.y+= abs(sin(-distancia*7.0*PI+time*10.0)*amplitud);
            // ************************************************************************
